...
func start(node_name, dialog_name):
	...
	match get_story_state():
		...
		9:
			...
			"choices":
				{
					"To ask her right away.": "on_rightaway",
					"To ask her later.": "on_later"
				}
			})

func on_rightaway(node_name, dialog_name):
	var cd = check_dialog(node_name, dialog_name, "on_rightaway")
...
func on_later(node_name, dialog_name):
	var cd = check_dialog(node_name, dialog_name, "on_later")
